...
class Application extends AbstractEntity {
	...
	public function setAccount(\App\Entity\Account $account){
	...
	public function populate($row) {
		foreach($row as $key => $value) {
			if ($key == "account_id") {
				$this->setAccount(
				    EntityManager::getRepository(\App\Entity\Account::class)->findById($value)
                );
			}
				...
				$this->getApplicationStatus()->setApplicationStatusId($value);
			}
			else if ($key == "subscription_id") {
				$this->getSubscription()->setSubscriptionId($value);
			}
			else if ($key == "submited_data") {
