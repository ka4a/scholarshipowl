...
class RelatedFieldsUpdateAction extends RestAction
{
    use RelatedAction;
    use CanHydrate;
    use CanValidate;
    ...
    public function handle($request)
    {
        ...
        $template->setFields(new ArrayCollection());
        foreach ($request->getData() as $raw) {
            /** @var ScholarshipTemplateField $field */
            $field = $this->hydrateEntity($this->related()->getClassName(), $raw);
            $template->addFields($field);
        }
