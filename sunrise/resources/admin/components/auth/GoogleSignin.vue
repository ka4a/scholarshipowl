<template>
  <a class="button is-primary google-button" @click="onClick">
    <figure class="google-button__icon">
      <svg viewBox="0 0 366 372" fill="white" width="18px" height="18px" xmlns="http://www.w3.org/2000/svg">
    ...
    <span class="google-button__text">{{ label }}</span>
  </a>
</template>
<script>
export default {
  ...
  methods: {
    onClick() {
      this.$store.dispatch('user/authenticate', { provider: 'google' })
        .then(() => {
          if (this.redirect) {
        ...
        .catch(() => this.$toast.open({ type: 'is-danger', message: 'Failed authorization' }));
    },
  },
};
</script>
