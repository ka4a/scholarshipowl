...
export default {
  ...
  methods: {
    ...
    onMoved({ application, to }) {
      ...
      return (new Store(ItemStore('application', { item: application, include: ['status'] })))
        .dispatch('save', { form })
        .then(application => this.store.dispatch('updateItem', { id: application.id, item: application }))
        .catch((error) => {
  ...
  computed: {
    ...
    next: function() {
      const next = this.current < this.waiting.length ? this.current : 0;
      if (this.current && this.waiting[next]) {
        if (this.waiting[next].id === this.$route.params.application) {
          return null;
        }
        return this.waiting[next].id;
      }
    },

    prev: function() {
      const prev = this.current - 2 > 0 ? this.current - 2 : this.waiting.length - 2;
      if (this.current && this.waiting[prev]) {
        return this.waiting[prev].id;
      }
    },
